...
class AsDiscreteTest {
    ...
    private fun makePlotSpec2(
        ...
        fun formatSpec(
            ...
            return values
                .filter { (storage, _) -> storage == targetStorage }
        ...
        val fooAnnotationSpec = """{"variable": "foo", "annotation": "discrete"}"""

        val barDataSpec = """"bar": [3, 3, 3, 4, 4, 4, 5, 5, 5]"""
        val barMappingSpec = """"fill": "bar""""
        val barAnnotationSpec = """{"variable": "bar", "annotation": "discrete"}"""
        ...
        fun formatAnnotationSpec(values: List<Pair<Storage, String>>, targetStorage: Storage): String? =
            formatSpec(values, targetStorage) { """"data_meta": {"series_annotation": [$it]}""" }
    ...
    fun mappingsAndDataMerging() {
        ...
        cases.forEach {
            makePlotSpec2("point", it.fooMapping, it.fooData, it.barData, it.barMapping)
                .let(::toClientPlotConfig)
                .assertDiscreteVar("foo") { it.toString() }
                .assertDiscreteVar("bar") { it.toString() }
        }
    }
