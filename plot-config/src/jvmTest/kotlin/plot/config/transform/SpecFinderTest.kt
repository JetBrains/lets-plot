...
class SpecFinderTest(private val myInput: Map<String, Any>, private val myKeys: Array<String>, private val myExpected: List<Map<String, Any>>) {
    ...
    companion object {
        ...
        fun params(): Collection<Array<Any>> {
            ...
            return listOf(arrayOf(emptyMap<Any, Any>(), arrayOf("a", "b", "c"), emptyList<Any>()), arrayOf(emptyMap<Any, Any>(), arrayOfNulls<String>(0), listOf(emptyMap<Any, Any>())), arrayOf(TARGET_SPEC, arrayOfNulls<String>(0), TARGET_LIST_OF_ONE), arrayOf(map(TARGET_SPEC, "_", "_"), arrayOf("0"), TARGET_LIST_OF_ONE), arrayOf(map("_", TARGET_SPEC, "_"), arrayOf("1"), TARGET_LIST_OF_ONE), arrayOf(map("_", "_", TARGET_SPEC), arrayOf("2"), TARGET_LIST_OF_ONE), arrayOf(map(TARGET_SPEC, TARGET_SPEC, TARGET_SPEC), arrayOf("2"), TARGET_LIST_OF_ONE), arrayOf(map(map(map("_", "_", TARGET_SPEC))), arrayOf("0", "0", "2"), TARGET_LIST_OF_ONE), arrayOf(map(list("_", map("_", "_", TARGET_SPEC), "_")), arrayOf("0", "2"), TARGET_LIST_OF_ONE), arrayOf(map(listContainingSpecsContainingTargetSpecs), arrayOf("0", "0"), listOf<Map<out Any, Any>>(TARGET_SPEC, TARGET_SPEC, TARGET_SPEC)), arrayOf(map(listContainingTargetSpecs), arrayOf("0"), listOf<Map<out Any, Any>>(TARGET_SPEC, TARGET_SPEC, TARGET_SPEC)))
        }
