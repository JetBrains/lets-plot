...
val artifactBaseName = "lets-plot-swing"
...
val jvmJarSwing by tasks.named<Jar>("jvmJar") {
...
publishing {
    publications {
        register("letsPlotJvmSwing", MavenPublication::class) {
            groupId = artifactGroupId
            artifactId = artifactBaseName
            version = artifactVersion

            artifact(jvmJarSwing)

            pom {
                name = "Lets-Plot for Swing"
                description = "Lets-Plot JVM package with Swing rendering"
...
signing {
    if (!project.version.toString().contains("SNAPSHOT")) {
        sign(publishing.publications["letsPlotJvmSwing"])
    }
}
