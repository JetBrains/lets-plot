...
class LinearBreaksHelperTest {
    ...
    fun breaksNormalRange() {
        assertBreaks(DOMAINS, EXPECTED_BREAKS, ERROR_TOLERANCE)
    }
    ...
    fun breaksLargeNumbers() {
        val factor = 1e5
        assertBreaks(
                multiply(DOMAINS, factor),
                multiply(EXPECTED_BREAKS, factor),
                ERROR_TOLERANCE)
    }
    ...
    companion object {
        ...
        private fun multiply(values2d: Array<DoubleArray>, factor: Double): Array<DoubleArray> {
            return values2d.map { values -> multiply(values, factor) }.toTypedArray()
        }
        ...
        private fun assertBreaks(scaleDomains: Array<DoubleArray>, expectedBreaks: Array<DoubleArray>, errorTolerance: Double) {
            ...
            for ((i, domain) in scaleDomains.withIndex()) {
                ...
                val breaks_i = computeBreaks(domain[0], domain[1], targetBreakCount)
                ...
                assertArrayEquals(
                        ...
                        asPrimitiveDoubles(breaks_i).toTypedArray(),
                        errorTolerance
                )
            }
        }
