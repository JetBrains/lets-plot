...
class DateTimeBreaksHelperTest {
    ...
    fun milliseconds1() {
        ...
        assertMilliseconds(expected, 100, 10)
    }

    @Test
    fun milliseconds2() {
        val expected = intArrayOf(50, 100)
        assertMilliseconds(expected, 100, 3)
    }
    ...
    fun minInterval_Day_lessTicks() {
        ...
        assertDays(expected, 5, 3, TimeInterval.days(1))
    }
    ...
    fun minInterval_Day_moreTicks() {
        ...
        assertDays(expected, 5, 10, TimeInterval.days(1))
    }

    @Test
    fun minInterval_Month_lessTicks() {
        val expected = intArrayOf(6, 9)
        assertMonths(expected, 6, 2, TimeInterval.months(1))
    }
    ...
    fun minInterval_Month_moreTicks() {
        ...
        assertMonths(expected, 5, 10, TimeInterval.months(1))
    }
    ...
    companion object {
        ...
        private val BASE_INSTANT = TimeZone.UTC.toInstant(DateTime(BASE_DATE, BASE_TIME))
        ...
        private fun assertDays(expected: IntArray, dCount: Long, targetBreakCount: Int, minInterval: TimeInterval? = null) {
            ...
            assertTimePartEquals(expected, breaks, DAYS)
        ...
        private fun assertMonths(expected: IntArray, mCount: Int, targetBreakCount: Int, minInterval: TimeInterval? = null) {
            ...
            val breaks = computeBreaks(instant1, instant2, targetBreakCount, minInterval)
            assertTimePartEquals(expected, breaks, MONTHS)
        ...
        private fun assertYears(expected: IntArray, yCount: Int, targetBreakCount: Int) {
            val date = Date(1, Month.APRIL, 2013)
            val dateTime1 = DateTime(date, BASE_TIME)
            val dateTime2 = DateTime(Date(1, Month.APRIL, date.year + yCount), BASE_TIME)

            val instant1 = TimeUtil.asInstantUTC(dateTime1)
            val instant2 = TimeUtil.asInstantUTC(dateTime2)

            val breaks = computeBreaks(instant1, instant2, targetBreakCount)
            assertTimePartEquals(expected, breaks, YEARS)

            assertTimesEqual(breaks, Time.DAY_START)
            assertDaysOfYearEqual(breaks, 0)
        }

        private fun computeBreaks(fromInstant: Long, toInstant: Long,
                                  targetBreakCount: Int,
                                  minInterval: TimeInterval? = null): Array<Double> {
            val helper = DateTimeBreaksHelper(fromInstant.toDouble(), toInstant.toDouble(), targetBreakCount, minInterval)
        ...
        private fun assertTimePartEquals(expected: IntArray, breaks: Array<Double>,
            ...
            val result = take(timePartExtractor, breaks)
