...
import jetbrains.datalore.plot.builder.tooltip.MathUtil.ClosestPointChecker
import jetbrains.datalore.plot.builder.tooltip.MathUtil.ClosestPointChecker.COMPARISON_RESULT
import jetbrains.datalore.plot.builder.tooltip.MathUtil.ClosestPointChecker.COMPARISON_RESULT.*
...
class ClosestPointCheckerTest {
    ...
    private fun check(coord: DoubleVector): jetbrains.datalore.plot.builder.tooltip.ClosestPointCheckerTest.Expectation<Boolean> {
        return jetbrains.datalore.plot.builder.tooltip.ClosestPointCheckerTest.Expectation { cp!!.check(coord) }
    }

    private fun compare(coord: DoubleVector): jetbrains.datalore.plot.builder.tooltip.ClosestPointCheckerTest.Expectation<COMPARISON_RESULT> {
        return jetbrains.datalore.plot.builder.tooltip.ClosestPointCheckerTest.Expectation { cp!!.compare(coord) }
    }
    ...
    private class Expectation<TRes> internal constructor(private val myCall: () -> TRes) : Runnable {
        ...
        internal fun assertResult(expected: TRes): jetbrains.datalore.plot.builder.tooltip.ClosestPointCheckerTest.Expectation<TRes> {
