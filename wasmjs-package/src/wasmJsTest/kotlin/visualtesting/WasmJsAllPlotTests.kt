...
class WasmJsAllPlotTests {
    @Test
    fun runAllPlotTests(): Promise<JsAny?> {
        val fontManager = createEmbeddedFontsManager()
        fontManager.installAllFontFaces()

        return documentFontsReady().then {
            WasmBitmapIO.preloadExpectedImages("visual-testing/plot").then {
                val canvasPeer = DomCanvasPeer(fontManager)
                val bitmapIO = WasmBitmapIO(subdir = "visual-testing/plot")
