...
class PolygonLocatorTest {
    private val viewport = Viewport(ViewportHelper(World.DOMAIN, true, myLoopY = false), ClientPoint(600, 400), 1, 15)
        .apply {
            zoom = 1
    ...
    @Test
    fun mouseInPolygon() {
        assertThat(locator.search(Vec(300.0, 200.0), entity, viewport)).isNotNull
    }

    @Test
    fun mouseOutOfPolygon() {
        assertThat(locator.search(Vec(0.0, 0.0), entity, viewport)).isNull()
    }
}