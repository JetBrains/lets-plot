...
internal class Path : Figure() {
    var fillRule: FillRule by variableAttr(FillRule.NON_ZERO)
    var pathData: Path2d? by variableAttr(null)
    ...
    companion object {
        val CLASS = ATTRIBUTE_REGISTRY.addClass(Path::class)

        val FillRuleAttrSpec = CLASS.registerVariableAttr(Path::fillRule)
        val PathDataAttrSpec = CLASS.registerVariableAttr(Path::pathData, affectsBBox = true)
    }
}
