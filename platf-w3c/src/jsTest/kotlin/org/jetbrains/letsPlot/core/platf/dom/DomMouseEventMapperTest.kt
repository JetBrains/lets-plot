...
class DomMouseEventMapperTest {
    ...
    fun wholeTarget() {
        ...
        assertEvent(div.dispatchEvent(DomEventType.MOUSE_ENTER, 0, 300)[m]!!.single(), MOUSE_ENTERED, 0, 300)
    }

    @Test
    fun clickEventsSequence() {
        val div = EventTargetAdapter(600, 600)
        val m = div.createEventMapper()
    ...
    fun dragReleaseShouldNotTriggerClickEvent() {
        ...
        div.dispatchEvent(DomEventType.MOUSE_MOVE, 310, 310).let { events ->
            assertEquals(2, events[m]!!.size)
            assertEvent(events[m]!![0], MOUSE_DRAGGED, 300, 300)
            assertEvent(events[m]!![1], MOUSE_DRAGGED, 310, 310)
        }
    ...
    private class EventTargetAdapter(
        ...
        fun dispatchDocumentEvent(
            ...
            events.clear()
            kotlinx.browser.document.dispatchEvent(event)
            return events.toMap() // copy
        }
    }
