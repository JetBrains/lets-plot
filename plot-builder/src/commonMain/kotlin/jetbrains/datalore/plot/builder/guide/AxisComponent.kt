...
class AxisComponent(length: Double, orientation: jetbrains.datalore.plot.builder.guide.Orientation) : SvgComponent() {
    ...
    private val orientation = ValueProperty<jetbrains.datalore.plot.builder.guide.Orientation?>(null)
    // theme
    ...
    private fun defTickLabelHorizontalAnchor(orientation: jetbrains.datalore.plot.builder.guide.Orientation): TextLabel.HorizontalAnchor {
        return when (orientation) {
            jetbrains.datalore.plot.builder.guide.Orientation.LEFT -> RIGHT
            jetbrains.datalore.plot.builder.guide.Orientation.RIGHT -> LEFT
            jetbrains.datalore.plot.builder.guide.Orientation.TOP, jetbrains.datalore.plot.builder.guide.Orientation.BOTTOM -> MIDDLE
        }
    }

    private fun defTickLabelVerticalAnchor(orientation: jetbrains.datalore.plot.builder.guide.Orientation): TextLabel.VerticalAnchor {
        when (orientation) {
            jetbrains.datalore.plot.builder.guide.Orientation.LEFT, jetbrains.datalore.plot.builder.guide.Orientation.RIGHT -> return CENTER
            jetbrains.datalore.plot.builder.guide.Orientation.TOP -> return BOTTOM
            jetbrains.datalore.plot.builder.guide.Orientation.BOTTOM -> return TOP
    ...
    private fun buildAxis() {
        ...
        when (orientation.get()) {
            jetbrains.datalore.plot.builder.guide.Orientation.LEFT, jetbrains.datalore.plot.builder.guide.Orientation.RIGHT -> {
                x2 = 0.0
                x1 = x2
                start = 0.0
                y1 = start
        ...
        if (breaksEnabled()) {
            ...
            if (!(breaks == null || breaks.isEmpty())) {
                ...
                for (br in breaks) {
                    ...
                    when (orientation.get()) {
                        jetbrains.datalore.plot.builder.guide.Orientation.LEFT, jetbrains.datalore.plot.builder.guide.Orientation.RIGHT -> transformTranslate(group, 0.0, br)
                        jetbrains.datalore.plot.builder.guide.Orientation.TOP, jetbrains.datalore.plot.builder.guide.Orientation.BOTTOM -> transformTranslate(group, br, 0.0)
                        else -> throw RuntimeException("Unexpected orientation:" + orientation.get())
                    }
    ...
    private fun buildTick(label: String, labelOffset: DoubleVector, gridLineLength: Double): SvgGElement {
        ...
        when (orientation.get()) {
            ...
            jetbrains.datalore.plot.builder.guide.Orientation.TOP -> {
                if (tickMark != null) {
                    tickMark.x2().set(0.0)
    ...
    fun tickLabelBaseOffset(): DoubleVector {
        ...
        when (orientation.get()) {
            jetbrains.datalore.plot.builder.guide.Orientation.LEFT -> return DoubleVector(-distance, 0.0)
            jetbrains.datalore.plot.builder.guide.Orientation.RIGHT -> return DoubleVector(distance, 0.0)
            jetbrains.datalore.plot.builder.guide.Orientation.TOP -> return DoubleVector(0.0, -distance)
            jetbrains.datalore.plot.builder.guide.Orientation.BOTTOM -> return DoubleVector(0.0, distance)
            else -> throw RuntimeException("Unexpected orientation:" + orientation.get())
        }
    }
