...
class TooltipSpecFactory(
    ...
    fun create(geomTarget: GeomTarget): List<jetbrains.datalore.plot.builder.interact.TooltipSpec> {
        return ArrayList(Helper(geomTarget).tooltipSpecs)
    }

    private inner class Helper(private val myGeomTarget: GeomTarget) {
        internal val tooltipSpecs = ArrayList<jetbrains.datalore.plot.builder.interact.TooltipSpec>()
        ...
        private fun addAxisTooltipSpec() {
            for (aes in myAxisAes) {
                if (isAxisTooltipAllowed(aes)) {
                    ...
                    tooltipSpecs.add(
                        jetbrains.datalore.plot.builder.interact.TooltipSpec(
                            layoutHint,
                            text,
                            layoutHint.color!!
                        )
        ...
        private fun isAxisTooltipAllowed(aes: Aes<*>): Boolean {
            ...
            return if (jetbrains.datalore.plot.builder.interact.TooltipSpecFactory.Companion.MAP_COORDINATE_NAMES.contains(label)) {
                false
        ...
        private fun removeAutoGeneratedMappings(aesListForTooltip: MutableList<Aes<*>>) {
            aesListForTooltip.removeAll { jetbrains.datalore.plot.builder.interact.TooltipSpecFactory.Companion.AUTO_GENERATED_NAMES.contains(getMappedData(it).label) }
        }
        ...
        private fun createHintForAxis(aes: Aes<*>): TipLayoutHint {
            if (aes === Aes.X) {
                return TipLayoutHint.xAxisTooltip(DoubleVector(tipLayoutHint().coord!!.x, axisOrigin.y),
                    jetbrains.datalore.plot.builder.interact.TooltipSpecFactory.Companion.AXIS_TOOLTIP_COLOR
                )
            }
        ...
        private fun applyTipLayoutHint(aes: List<Aes<*>>, layoutHint: TipLayoutHint) {
            ...
            tooltipSpecs.add(jetbrains.datalore.plot.builder.interact.TooltipSpec(layoutHint, text, fill))
