...
internal object PlotGuidesAssemblerUtil {
    ...
    fun guideDataRangeByAes(stitchedLayers: jetbrains.datalore.plot.builder.assemble.StitchedPlotLayers, guideOptionsMap: Map<Aes<*>, GuideOptions>): Map<Aes<*>, ClosedRange<Double>> {
        val m = HashMap<Aes<*>, ClosedRange<Double>>()
        val aesSet = jetbrains.datalore.plot.builder.assemble.PlotGuidesAssemblerUtil.mappedRenderedAesToCreateGuides(
            stitchedLayers,
            guideOptionsMap
        )
        for (aes in aesSet) {
    ...
    fun createColorBarAssembler(scaleName: String,
                                ...
                                theme: LegendTheme): jetbrains.datalore.plot.builder.assemble.ColorBarAssembler {

        val domain = dataRangeByAes[aes]
        checkState(domain != null, "Data range is not defined for aes $aes")
        val result = jetbrains.datalore.plot.builder.assemble.ColorBarAssembler(scaleName, domain!!, scale, theme)
