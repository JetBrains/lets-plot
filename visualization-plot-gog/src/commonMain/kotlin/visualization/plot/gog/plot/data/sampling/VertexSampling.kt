...
import jetbrains.datalore.visualization.plot.base.data.sampling.SamplingUtil.calculateRingLimits
import jetbrains.datalore.visualization.plot.base.data.sampling.SamplingUtil.getRingIndex
import jetbrains.datalore.visualization.plot.base.data.sampling.SamplingUtil.getRingLimit
...
internal abstract class VertexSampling(sampleSize: Int) : SamplingBase(sampleSize), PointSampling {
    ...
    override fun apply(population: DataFrame): DataFrame {
        ...
        val limits = if (rings.size == 1 && !isClosed(rings.get(0)))
            listOf(sampleSize)
        else
