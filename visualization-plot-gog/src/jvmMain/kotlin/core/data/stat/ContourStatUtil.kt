...
object ContourStatUtil {
    ...
    fun computeLevels(data: DataFrame, binOptions: StatUtil.BinOptions): List<Double>? {
        ...
        return ContourStatUtil.computeLevels(zRange, binOptions)
    }
    ...
    fun computeContours(
        ...
        for (level in levels) {
            val levelSegments = computeSegments(colCount, rowCount, data, level)
            val paths = joinSegments(levelSegments)
            // checkEdges(paths, colCount, rowCount); // debug use
            var pathsReal = convertPaths(paths, xStep, yStep, origin, level)
        ...
        for (row in 0 until rowCount - 1) {
            for (col in 0 until colCount - 1) {
                ...
                if (level > min && level <= max) {
                    ...
                    val cellSegments = computeSegmentsForGridCell(level, v, col, row)
    ...
    private fun computeSegmentsForGridCell(level: Double?, value5: DoubleArray, col: Int, row: Int): List<Pair<Edge, Edge>> {
        ...
        for (i in 0..4) {
            points.add(TripleVector(col + xLoc[i], row + yLoc[i], value5[i]))
        }
    ...
    private class TripleVector internal constructor(x: Double, y: Double, val z: Double) {
        ...
        override fun equals(o: Any?): Boolean {
            ...
            val that = o as TripleVector?
            return myX == that!!.myX && myY == that.myY && myIsCenter == that.myIsCenter
        }

        override fun hashCode(): Int {
            return Objects.hash(myX, myY, myIsCenter)
        }
