...
import jetbrains.datalore.visualization.plot.gog.core.data.stat.ContourStatUtil.removePathByEndpoints
...
class ContourFillHelper(xRange: ClosedRange<Double>, yRange: ClosedRange<Double>) {
    ...
    fun createPolygons(
        ...
        for (i in 0 until n + 1) {
            ...
            if (i > 0) {
                polygonPieces.addAll(reverseAll(pathByLevels[levels[i - 1]]!!))
            }
            if (i < n) {
    ...
    private fun createOuterMap(paths: List<List<DoubleVector>>, cornerPoints: MutableList<DoubleVector>): Map<DoubleVector, DoubleVector> {
        ...
        for (point in points) {
            ...
            when (kind) {
                ContourFillHelper.BorderKind.DOWN -> list[0].add(point)
                ContourFillHelper.BorderKind.RIGHT -> list[1].add(point)
                ContourFillHelper.BorderKind.UP -> list[2].add(point)
                ContourFillHelper.BorderKind.LEFT -> list[3].add(point)
            }
        }

        list[0].sortWith(Comparator { o1, o2 -> java.lang.Double.compare(o1.x, o2.x) })
        list[1].sortWith(Comparator { o1, o2 -> java.lang.Double.compare(o1.y, o2.y) })
        list[2].sortWith(Comparator { o1, o2 -> java.lang.Double.compare(o2.x, o1.x) })
        list[3].sortWith(Comparator { o1, o2 -> java.lang.Double.compare(o2.y, o1.y) })
    ...
    private fun getKind(point: DoubleVector): BorderKind {
        return if (belowOrOnLine(myLowLeft, myUpRight, point) && belowOrOnLine(myUpLeft, myLowRight, point)) {
            BorderKind.DOWN
        } else if (belowOrOnLine(myLowLeft, myUpRight, point) && !belowOrOnLine(myUpLeft, myLowRight, point)) {
            BorderKind.RIGHT
        } else if (!belowOrOnLine(myLowLeft, myUpRight, point) && !belowOrOnLine(myUpLeft, myLowRight, point)) {
            BorderKind.UP
        } else if (!belowOrOnLine(myLowLeft, myUpRight, point) && belowOrOnLine(myUpLeft, myLowRight, point)) {
    ...
    companion object {
        ...
        fun computeFillLevels(zRange: ClosedRange<Double>, levels: List<Double>): List<Double> {
            ...
            for (i in 0 until levels.size - 1) {
                fillLevels.add(java.lang.Double.sum(levels[i], levels[i + 1]) / 2.0)
            }
