...
object StatUtil {
    ...
    fun weightVector(dataLength: Int, data: DataFrame): List<Double> {
        ...
        } else Collections.nCopies(dataLength, 1.0)
    }
    ...
    fun binCountAndWidth(dataRange: Double, binOptions: BinOptions): CountAndWidth {
        ...
        if (binOptions.hasBinWidth()) {
            ...
            count = Math.min(MAX_BIN_COUNT.toDouble(), count)
            binCount = Math.ceil(count).toInt()
        } else {
  ...
  */
    ...
    fun computeBins(
        ...
        for (dataIndex in valuesX.indices) {
            ...
            totalCount += weight
            val binIndex = Math.floor((x - startX) / binWidth).toInt()
    ...
    class BinOptions(binCount: Int, val binWidth: Double?  // optional
        ...
        init {
            this.binCount = Math.min(MAX_BIN_COUNT, Math.max(1, binCount))
        }
