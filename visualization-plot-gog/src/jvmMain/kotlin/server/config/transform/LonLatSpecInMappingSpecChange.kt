...
import jetbrains.datalore.visualization.plot.gog.config.Option.Mapping
...
internal class LonLatSpecInMappingSpecChange : SpecChange {
    ...
    override fun apply(spec: MutableMap<String, Any>, ctx: SpecChangeContext) {
        ...
        if (mapIdMapping.containsKey(LONLAT_SPEC_KEY)
            ...
            aesSpec[Mapping.MAP_ID] = GENERATED_LONLAT_COLUMN_NAME
        }
    }

    companion object {
        private const val GEO_COORD_FORMAT = "%s, %s"
        ...
        private fun concatColumns(lon: List<*>, lat: List<*>): List<String> {
            ...
            val n = lon.size
            while (i < n) {
                coordinates.add(concatCoordinates(lon[i], lat[i]))
                ++i
            }
