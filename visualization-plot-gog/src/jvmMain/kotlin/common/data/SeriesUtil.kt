...
object SeriesUtil {
    ...
    val NEGATIVE_NUMBER = Predicate<Double> { input -> input < 0 }
    ...
    fun isFinite(v: Double?): Boolean {
        return v != null && java.lang.Double.isFinite(v)
    }

    fun isFinite(v: Double): Boolean {
        return java.lang.Double.isFinite(v)
    }
    ...
    fun allFinite(v0: Double?, v1: Double?, v2: Double?): Boolean {
        return allFinite(v0, v1) && isFinite(v2)
    }
    ...
    fun mean(values: List<Double>, defaultValue: Double?): Double? {
        ...
        var i = -1.0
        for (value in values) {
            if (value != null && java.lang.Double.isFinite(value)) {
                i++
    ...
    open class CheckedDoubleIterable(private val myIterable: Iterable<*>) {
        ...
        init {
            myEmpty = Iterables.isEmpty(myIterable)
            if (myEmpty) {
                myCanBeCast = true
            } else {
                myCanBeCast = all(filter(myIterable) { Objects.nonNull(it) }) { input -> input is Double }
            }
        }
