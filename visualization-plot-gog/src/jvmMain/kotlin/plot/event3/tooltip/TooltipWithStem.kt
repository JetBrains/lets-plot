...
class TooltipWithStem : SvgComponent() {
    private val myTooltipFrame: TooltipFrame
    private val myStemFootingLine: SvgLineElement
    ...
    init {
        myTooltipFrame = TooltipFrame()

        myStemFootingLine = SvgLineElement()
    ...
    private fun updateStem() {
        ...
        if (isHorizontal(dir)) {
            val stemFootingX: Double
            if (dir == StemDirection.LEFT) {
                stemFootingX = contentRect.origin.x
            } else {
                stemFootingX = contentRect.right
            }
            ...
            val stemFootingY: Double
            if (dir == StemDirection.UP) {
                stemFootingY = contentRect.top
            } else {
                stemFootingY = contentRect.bottom
            }
    ...
    companion object {
        ...
        private val MAX_STEM_FOOTING_LENGTH = 12.0
        private val STEM_FOOTING_TO_SIDE_LENGTH_RATIO = 0.4
        private val SIDE_LENGTH_TO_STEM_FOOTING_RATIO = (1.0 - STEM_FOOTING_TO_SIDE_LENGTH_RATIO) / 2
        private val STEM_FOOTING_STROKE_WIDTH = TooltipFrame.STROKE_WIDTH + 2
    }
