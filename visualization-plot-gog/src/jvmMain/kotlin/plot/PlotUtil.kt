...
object PlotUtil {
    ...
    private fun computeLayerDryRunRangeAfterSizeExpand(
        ...
        val minMax = doubleArrayOf(java.lang.Double.POSITIVE_INFINITY, java.lang.Double.NEGATIVE_INFINITY)
    ...
    private fun updateExpandedMinMax(value: Double, expand: Double, expandedMinMax: DoubleArray) {
        expandedMinMax[0] = Math.min(value - expand, expandedMinMax[0])
        expandedMinMax[1] = Math.max(value + expand, expandedMinMax[1])
    }
    ...
    internal fun createLayerAesthetics(layer: GeomLayer,
        ...
        for (aes in layer.renderedAes()) {
            ...
            if (layer.hasConstant(aes)) {
                ...
                val mapperOption1 = mapperOption as Function<Double, Any>?
    ...
    private fun <T> asAesValue(aes: Aes<T>, dataValue: T, mapperOption: Function<Double, T>?): T {
        return if (aes.isNumeric && mapperOption != null) {
            mapperOption.apply(dataValue as Double)
        } else dataValue
    }
    ...
    fun rangeWithExpand(layer: GeomLayer, aes: Aes<Double>, range: ClosedRange<Double>?): ClosedRange<Double>? {
        ...
        if (layer.rangeIncludesZero(aes)) {
            ...
            if (lowerEndpoint == 0.0 ||
                    upperEndpoint == 0.0 ||
                    Math.signum(lowerEndpoint) == Math.signum(upperEndpoint)) {
                if (lowerEndpoint >= 0) {
                    lowerExpand = 0.0
                } else {
                    upperExpand = 0.0
