...
class LegendComponentSpec(title: String, val breaks: List<LegendBreak>, theme: LegendTheme) : LegendBoxSpec(title, theme) {
    ...
    private fun pretty(v: DoubleVector): DoubleVector {
        val margin = 1.0
        return DoubleVector(
                Math.floor(v.x / 2) * 2 + 1.0 + margin,
                Math.floor(v.y / 2) * 2 + 1.0 + margin
        )
    }
        ...
        if (myLayout == null) {
            ...
            if (myOptions.isByRow) {
                if (myOptions.hasColCount()) {
                    colCount = Math.min(myOptions.colCount, breakCount)
                } else if (myOptions.hasRowCount()) {
                    colCount = Math.ceil(breakCount / myOptions.rowCount.toDouble()).toInt()
                } else if (legendDirection === LegendDirection.HORIZONTAL) {
                    colCount = breakCount
                } else {
                    colCount = 1
                }
                rowCount = Math.ceil(breakCount / colCount.toDouble()).toInt()
            } else {
                // by column
                if (myOptions.hasRowCount()) {
                    rowCount = Math.min(myOptions.rowCount, breakCount)
                } else if (myOptions.hasColCount()) {
                    rowCount = Math.ceil(breakCount / myOptions.colCount.toDouble()).toInt()
                } else if (legendDirection !== LegendDirection.HORIZONTAL) {
                    rowCount = breakCount
                } else {
                    rowCount = 1
                }
                colCount = Math.ceil(breakCount / rowCount.toDouble()).toInt()
            }
