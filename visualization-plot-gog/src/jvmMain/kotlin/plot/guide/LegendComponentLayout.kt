...
internal abstract class LegendComponentLayout(title: String, protected val breaks: List<LegendBreak>, val keySize: DoubleVector, legendDirection: LegendDirection) : LegendBoxLayout(title, legendDirection) {
    ...
    private fun doLayout() {
        val labelHeight = LegendBoxLayout.LABEL_SPEC.height()
        val labelLeftMargin = LegendBoxLayout.LABEL_SPEC.width(1) / 2
    ...
    private class MyHorizontal internal constructor(title: String, breaks: List<LegendBreak>, keySize: DoubleVector) : LegendComponentLayout(title, breaks, keySize, LegendDirection.HORIZONTAL) {
        init {
        ...
        override fun labelSize(index: Int): DoubleVector {
            ...
            return DoubleVector(LegendBoxLayout.LABEL_SPEC.width(label.length), LegendBoxLayout.LABEL_SPEC.height())
        }
    }

    private class MyHorizontalMultiRow internal constructor(title: String, breaks: List<LegendBreak>, keySize: DoubleVector) : MyMultiRow(title, breaks, keySize, LegendDirection.HORIZONTAL) {
        init {
            colCount = breaks.size
            rowCount = 1
        }
    ...
    private class MyVertical internal constructor(title: String, breaks: List<LegendBreak>, keySize: DoubleVector) : MyMultiRow(title, breaks, keySize, LegendDirection.VERTICAL) {
        init {
            colCount = 1
    ...
    private abstract class MyMultiRow internal constructor(title: String, breaks: List<LegendBreak>, keySize: DoubleVector, legendDirection: LegendDirection) : LegendComponentLayout(title, breaks, keySize, legendDirection) {
        ...
        init {
            for (br in breaks) {
                myMaxLabelWidth = Math.max(myMaxLabelWidth, LegendBoxLayout.LABEL_SPEC.width(br.label.length))
            }
        }
