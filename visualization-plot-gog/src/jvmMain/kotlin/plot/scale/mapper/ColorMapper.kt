...
object ColorMapper {
    ...
    fun gradientHSV(
        ...
        if (autoHueDirection) {
            val dH = Math.abs(highHue - lowHue)
            if (dH > 180) {
                if (highHue >= lowHue) {
                    lowHue += 360.0
                } else {
        ...
        val mapperH = Mappers.linear(domain, lowHue, highHue, java.lang.Double.NaN)
        val mapperS = Mappers.linear(domain, lowS, highS, java.lang.Double.NaN)
        val mapperV = Mappers.linear(domain, lowHSV[2], highHSV[2], java.lang.Double.NaN)
