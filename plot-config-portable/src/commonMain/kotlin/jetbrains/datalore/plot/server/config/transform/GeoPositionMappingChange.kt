...
class GeoPositionMappingChange : SpecChange {
    ...
    override fun apply(spec: MutableMap<String, Any>, ctx: SpecChangeContext) {
        spec[GEO_POSITIONS] = HashMap(
            SUPPORTED_TAGS[getFirstSupportedTagInMapMeta(
                spec
            )]?.invoke(spec)!!)
    }

    override fun isApplicable(spec: Map<String, Any>): Boolean {
        return getFirstSupportedTagInMapMeta(
            spec
        ) != null
    }
    ...
    companion object {
        private val SUPPORTED_TAGS = mapOf<String, (Map<String, Any?>) -> Map<String, Any?>>(
                GeoReference.TAG to { map ->
                    transformGeoReference(
                        map
                    )
                },
                GeoDataFrame.TAG to { map ->
                    transformGeoDataFrame(
                        map
                    )
                }
        )
        ...
        private fun getFirstSupportedTagInMapMeta(geomSpec: Map<String, Any>): String? {
            return if (geomSpec[MAP_DATA_META] is Map<*, *>) {
                getFirstSupportedValue(
                    (geomSpec[MAP_DATA_META] as Map<*, *>).keys,
                    SUPPORTED_TAGS.keys
                )
            } else {
                null
