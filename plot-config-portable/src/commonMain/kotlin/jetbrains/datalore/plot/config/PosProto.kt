...
internal object PosProto {
    ...
    fun createPosProvider(posName: String, options: Map<*, *>): PosProvider {
        ...
        return when (posName) {
            ...
            JITTER -> PosProvider.jitter(opts.getDouble(JITTER_WIDTH), opts.getDouble(
                JITTER_HEIGHT
            ))
            NUDGE -> PosProvider.nudge(opts.getDouble(NUDGE_WIDTH), opts.getDouble(
                NUDGE_HEIGHT
            ))
            JITTER_DODGE -> PosProvider.jitterDodge(opts.getDouble(JD_DODGE_WIDTH), opts.getDouble(
                JD_JITTER_WIDTH
