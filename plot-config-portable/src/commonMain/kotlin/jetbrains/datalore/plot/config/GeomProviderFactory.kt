...
internal object GeomProviderFactory {
    private val PROVIDER = HashMap<GeomKind, (GeomProvider.Context) -> GeomProvider>()
    ...
    fun createGeomProvider(geomKind: GeomKind, layerConfig: OptionsAccessor): (GeomProvider.Context) -> GeomProvider {
        return when (geomKind) {
            ...
            GeomKind.PIE -> GeomProvider.pie {
                ...
                layerConfig.getDouble(Option.Geom.Pie.STROKE)?.let { geom.strokeWidth = it }
                layerConfig.getColor(Option.Geom.Pie.STROKE_COLOR)?.let { geom.strokeColor = it }
                geom
            }
