#
# Copyright (c) 2019. JetBrains s.r.o.
# Use of this source code is governed by the MIT license that can be found in the LICENSE file.
#

import numpy as np
import pytest

from lets_plot.plot.geom_imshow_ import geom_imshow

try:
    from palettable.matplotlib import matplotlib as palettable
except ImportError:
    palettable = None


# See notebook:
#       docs/testing/testing_imshow_nan_values.ipynb

def _image_spec(href, width, height):
    return dict(
        data_meta={},
        geom='image',
        href=href,
        mapping={},
        show_legend=True,
        xmin=-0.5,
        ymin=-0.5,
        xmax=width - 1 + 0.5,
        ymax=height - 1 + 0.5
    )


def _image_with_color_grey_scale_spec(href, width, height, data_min, data_max):
    layer_spec = _image_spec(href, width, height)
    layer_spec['color_by'] = 'paint_c'
    layer_spec['mapping'] = dict(paint_c=[data_min, data_max])
    scale_spec = dict(
        aesthetic='paint_c',
        start=0,
        end=1,
        name='',
        scale_mapper_kind='color_grey'
    )
    return {
        'feature-list': [
            dict(layer=layer_spec),
            dict(scale=scale_spec),
        ]
    }


def _image_with_color_gradientn_scale_spec(href, width, height, data_min, data_max, colors):
    layer_spec = _image_spec(href, width, height)
    layer_spec['color_by'] = 'paint_c'
    layer_spec['mapping'] = dict(paint_c=[data_min, data_max])
    scale_spec = dict(
        aesthetic='paint_c',
        colors=colors,
        name='',
        scale_mapper_kind='color_gradientn',
    )
    return {
        'feature-list': [
            dict(layer=layer_spec),
            dict(scale=scale_spec),
        ]
    }


class Test:
    test_params_list = []

    expected_gray_2_x_3 = _image_with_color_grey_scale_spec(
        'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAMAAAACCAQAAAA3fa6RAAAAFUlEQVR4nGNg+M/A8B+IGUL/M/wHACJBBVE68QEEAAAAAElFTkSuQmCC',
        width=3,
        height=2,
        data_min=50,
        data_max=200
    )

    # 2 x 3 array of floats containing NaN
    test_params_list.append((
        np.array([
            [50., np.nan, 200.],
            [np.nan, 100., 50.]
        ]),
        None,  # cmap
        expected_gray_2_x_3
    ))

    # with 'cmap'
    expected_gray_2_x_3_magma = _image_with_color_gradientn_scale_spec(
        'data:image/png;base64,',
        width=3,
        height=2,
        data_min=50,
        data_max=200,
        colors=palettable.get_map("magma_32").hex_colors
    )

    # 2 x 3 array of floats containing NaN, cmap="magma"
    test_params_list.append((
        np.array([
            [50., np.nan, 200.],
            [np.nan, 100., 50.]
        ]),
        "magma",  # cmap
        expected_gray_2_x_3_magma
    ))

    @pytest.mark.parametrize('image_data, cmap, expected', test_params_list)
    def test_image_spec(self, image_data, cmap, expected):
        image_data.flags.writeable = False

        # spec = geom_imshow(image_data, extent=_extent)
        spec = geom_imshow(image_data, cmap=cmap)
        assert spec.as_dict() == expected
