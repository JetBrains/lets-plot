...
def test_geo_limit_response():
    ...
    data_frame: GeoDataFrame = LimitsGeoDataFrame().to_data_frame(response.features)
...
def test_geo_boundaries_point_response():
    ...
    assert_success_response(response)
    boundary: GeoDataFrame = BoundariesGeoDataFrame().to_data_frame(response.features)
...
def assert_geo_boundary(boundary: GeoDataFrame, index: int, polygon: GJPolygon, name=NAME, found_name=FOUND_NAME):
