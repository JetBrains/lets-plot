...
object TextAnchoring {
    internal fun offsetEmBoxTop(textLayout: TextLayout, fontSize: Double): Double {
        val first = textLayout.firstLineMetrics
        return first.ascent - EM_BOX_TOP_NUDGE * fontSize
    }

    internal fun offsetCap(vjust: Double, textLayout: TextLayout, fontSize: Double): Double {
        val first = textLayout.firstLineMetrics
        val totalHeight = textLayout.totalHeight
        return first.ascent + (vjust - 1) * totalHeight - CAP_VJUST_NUDGE * vjust * fontSize
    }

    fun offsetCap(anchor: VerticalAnchor, textLayout: TextLayout, fontSize: Double): Double {
