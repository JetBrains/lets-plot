...
import org.jetbrains.letsPlot.core.plot.base.tooltip.layout.LayoutManager.Companion.moveIntoLimit
import org.jetbrains.letsPlot.core.plot.base.tooltip.layout.LayoutManager.PositionedTooltip

class HorizontalTooltipExpander(
    private val margin: Double,
    private val mySpace: DoubleSpan
) {
        ...
        repeat(50) {
            groups
                ...
                ?.let { (g1, g2) ->
                    ...
                    groups[index] = Group(g1.tooltips + g2.tooltips, margin, mySpace)
                    groups.remove(g2)
                }
    ...
    internal class Group internal constructor(
        ...
        constructor(tooltip: PositionedTooltip, margin: Double, space: DoubleSpan) : this(listOf(tooltip), margin, space)

        internal val range: DoubleSpan

        init {
            val height = tooltips.sumOf(PositionedTooltip::height) + (tooltips.size - 1) * margin
        ...
        fun overlaps(other: Group): Boolean =
            DoubleSpan(
                range.lowerEnd - margin,
                range.upperEnd + margin
