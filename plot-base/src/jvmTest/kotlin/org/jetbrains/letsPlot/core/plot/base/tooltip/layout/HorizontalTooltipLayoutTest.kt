...
internal class HorizontalTooltipLayoutTest : TooltipLayoutTestBase() {
    ...
    fun whenOverlappedAfterArrange_ShouldRearrangeOverlappedAgain() {
        ...
        val layoutManagerController = createTipLayoutManagerBuilder(DoubleRectangle(0.0, 0.0, 500.0, 800.0))
            .addTooltip(
                tooltipBuilder.horizontal(IGNORED_KEY, coord(20.0, 200.0)).size(tipWidth, 40.0).buildTooltip()
            )   // tip1 is not overlapped right now
            .addTooltip(
                tooltipBuilder.horizontal(IGNORED_KEY, coord(20.0, 400.0)).size(tipWidth, 200.0).buildTooltip()
            ) // tip2 and tip3 are overlapped
