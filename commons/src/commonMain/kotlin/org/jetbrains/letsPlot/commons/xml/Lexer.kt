...
internal class Lexer(
    ...
    var token: Token = Token.Companion.EOF
        private set
    ...
    private fun nextTokenImpl(): Token {
        if (pos >= input.length) return Token.Companion.EOF

        return when (val c = peek()) {
            null -> Token.Companion.EOF
            '<' -> {
                advance()
                when {
                    peek() == '/' -> Token.Companion.LT_SLASH.also { advance() }
                    else -> Token.Companion.LT
                }
            }

            '>' -> Token.Companion.GT.also { advance() }
            '/' -> {
                advance()
                when {
                    peek() == '>' -> Token.Companion.SLASH_GT.also { advance() }
                    else -> Token.Companion.SLASH
                }
            }

            '=' -> Token.Companion.EQUALS.also { advance() }
